...
This vignette also makes uses of `ggplot2`.

## Data

The `RtGam` package requires a time series of observed incidence data.
...
If you want to select a smoothing or penalty basis dimension you can provide those to the model.
...
For example, we can refit the model to be non-adaptive by setting `m` to 1.
...
The `predict()` method draws samples from the model's fitted posterior distribution.
By default it draws 100 sampled timeseries of the posterior predicted cases.

```{r predict obs cases}
head(predict(fit))
```

Calling `plot` on the fitted model object also draws samples and plots them against the provided timeseries of cases.
...
It also requires an estimate of the generation interval, which needs to be draws from the literature or estimated separately using data on paired cases.
RtGam expects the GI to be formatted as a probability mass function.
We recommend using the package [primarycensored](https://primarycensored.epinowcast.org) to format a generation interval distribution as a PMF.

In this simulated example, cases are observed immediately so the mean delay is 0.
We have pre-generated the generation interval pmf (see more with `?sir_gt_pmf`).
