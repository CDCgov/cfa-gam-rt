...
fit_RTMB_convolve <- function(form, family, data, REML = TRUE,
  ...
  m <- glmmTMB::glmmTMB(
    form = form, family = family, data = data,
...
simfit <- function(delay_pmf, nt = 100, theta_start = 3, seed = 101, k = 20) {
  ...
  gam_fit <- gam(value ~ s(rdate, k = k),
    family = "nb", data = dd0, method = "REML",
    na.action = na.exclude
  ) ## na.exclude includes initial rdates
  gam_pred0 <- predict(gam_fit, type = "link", se.fit = TRUE)
  gam_pred <- with(gam_pred0, tibble(
    rdate = dd0$rdate,
