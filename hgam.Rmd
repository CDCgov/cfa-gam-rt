...
The `simulate_sir()` function generates a number of outputs in long format:
...
It currently doesn't generate reporting triangle data.
Here I will extend it to do so.
For now assume the same `reporting_delay_pmf` for all data (with the same maximum length):
...
For the spatial structure, create a four-by-four grid of mock counties:
...
I considered the following spatio-temporal covariance structures:
...
* $w_{st}$ is a spatio-temporal interaction (currently independent by space)
...
One of the features of the data is that the values of $R_t$ can be high enough that the susceptible population is depleted.
This isn't very realistic for respiratory infections so it could be something we look to improve in future.
...
We now use the `prep_model_df()` function to prepare the data for input to the model fitting function.
This function filters the data in a particular way, which it's important to be aware of.
**Important** A limitation of the work here is that the final report date does not include all data.
This could be something to fix.
...
Because of the filtering approach combined with not including all reference dates for the final report date, some reference dates are not included in the model currently:
...
We fit the model with `gamtools::fit_model`!
The calls to `n_distinct` and `interaction` below were to check the data (as warnings are thrown about missing factor levels).
...
The forecast recovery seems pretty reasonable, but can't handle the drop to zero (dupleted susceptibles):
...
The $R_t$ recovery is more questionable, that the model cannot currently handle the shape of the true $R_t$ values (and it can't deal with the depletion of susceptibles either):
